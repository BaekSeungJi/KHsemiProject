			...
			<div class="container-fluid">
				...
				<div align="center" style="margin-bottom: 100px">
					...
					<table border="1" bgcolor="#ffffff">
						...
						<tr>
							<td class="t">아이디</td>
							<td><%=dto.getId()%> <input type="hidden" name="command"
								value="pds_delete"> <input type="hidden" name="seq"
								value="<%=dto.getSeq()%>"></td>
						</tr>
						...
						<tr>
							<td class="t">내용</td>
							<td><textarea rows="20" cols="50" readonly="readonly"><%=dto.getContent()%></textarea>
							</td>
						</tr>

						<tr align="center">
							<td colspan="2" class="spe">
								<%
									if (user.getId().equals(dto.getId())) {
								%>
								<button
									onclick="location.href='PdsControl?command=pdsDelete&seq=<%=dto.getSeq()%>'">삭제</button>
								<button
									onclick="location.href='PdsControl?command=pdsUpdateGo&seq=<%=dto.getSeq()%>'">수정</button>
								<%
									}
								%>
								<button onclick="location.href='PdsControl?command=ad_noticeGo'">뒤로가기</button>
								<button
									onclick="location.href='ad_answerWrite.jsp?seq=<%=dto.getSeq()%>'">답글달기</button>
							</td>
						</tr>
