	...
	<section>
      ...
      %>
							...
							<tr onmouseover="this.style.background='#f0f0f0'"
								...
								<td><input type="hidden" name="command"
									value="ad_reserveUpdate"> <input type="hidden" id="id"
									value="<%=dto.getId()%>"> <input type="text"
									value="<%=dto.getCheckin() %>" id="checkin"></td>

								<td><input type="text" value="<%=dto.getCheckout() %>"
									id="checkout"></td>
								</td>
								<td><%=dto.getHotelname() %></td>
								<td><input type="text" value="<%=dto.getRequest() %>"
									id="request"></td>
								<td><%=dto.getRegdate() %></td>
								<td>
									<%if(dto.getDel() == 0){
        	  %><button
										onclick="location.href='ad_memberUpdateGo.jsp?seq=<%=dto.getSeq() %>&id=<%=dto.getId()%>'">수정</button>
          ...
          %>
        	  ...
        	  %>
									<button
										onclick="location.href='ReserveControl?command=ad_reserveDelete&seq=<%=dto.getSeq() %>&id=<%=dto.getId()%>'">예약취소</button>
      ...
      %>
        	  ...
        	  %><input type="button" value="삭제"
										onclick="location.href='ReviewControl?command=ad_reviewDelete&seq=<%=dto2.getNum() %>&id=<%=dto2.getId()%>'">
										<%  }else{
        	%> 삭제된 후기입니다 <%
      		 }
          %>
