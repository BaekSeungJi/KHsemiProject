	...
	<aside>
		<nav class="rad-sidebar rad-nav-min">
			<ul>
				...
				</a> <%}else{ %> <a href="MemberControl?command=ad_memberDeleteGo"
					class="inbox"> <i class="fas fa-user-alt"><span
							class="icon-bg rad-bg-success"></span></i> <span
						class="rad-sidebar-item">회원 관리</span>
				</a> <%} %>
				...
				<li>
					<%if(mem.getAuth()==2){ %> <a href="HotelControl?command=ad_hotel">
						<i class="fas fa-hotel"> <span class="icon-bg rad-bg-danger"></span>
					</i> <span class="rad-sidebar-item">호텔 관리</span>
				</a> <%}else{ %> <a href="PdsControl?command=ad_noticeGo" class="done"><i
	...
	<section>
      ...
      %>
          ...
          %><input type="button" value="삭제"
								onclick="location.href='MemberControl?command=memberblack&id=<%=dto.getId()%>'">
								<%}else{ %> 블랙리스트 회원입니다 <%} %>
							</td>
       ...
       %>
			...
			<form action="MemberControl">
				<div align="center" style="margin-top: 20px;">
					...
					<input type="hidden" name="id" value="<%=user.getId()%>"> <select
						name="sel">
