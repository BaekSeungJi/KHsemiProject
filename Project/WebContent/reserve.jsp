...
<%!// nvl 함수
	...
	//날짜를 클릭하면, 그날의 일정이 모두 보이게 하는 reservelist.jsp로 이동시키는 함수
	public String reservelist(int year, int month, int day) {
		String s = "";

		s += String.format("<a href='%s?year=%d&month=%d&day=%d'>", "reservelist.jsp", year, month, day);
	...
	// 일정을 기입하기 위해서 pen이미지를 클릭하면, reservewrite.jsp로 이동
	...
	public String makeTable(int year, int month, int day, List<ReserveDto> list) {
		...
		for (ReserveDto dto : list) {
			if (dto.getCheckin().substring(0, 8).equals(dates)) {

				s += "<tr bgcolor='pink'>";
				s += "<td>";
				s += "<a href='reservedetail.jsp?seq=" + dto.getSeq() + "'>";
				s += "<font style='font-size:8; color:red'>";
				s += dot3(dto.getHotelname());
				s += "</font>";
				s += "</a>";
				s += "</td>";
				s += "</tr>";
...
<body>
	...
	<%
		...
		String pp = String.format("<a href='%s?year=%d&month=%d'>" + "<img src='image/left.gif'></a>",
				"reserve.jsp", year - 1, month);

		// <
		String p = String.format("<a href='%s?year=%d&month=%d'>" + "<img src='image/prec.gif'></a>", "reserve.jsp",
				year, month - 1);

		// >
		String n = String.format("<a href='%s?year=%d&month=%d'>" + "<img src='image/next.gif'></a>", "reserve.jsp",
				year, month + 1);
	...
	<div align="center">
		...
		<table border="1">
			...
			<tr height="100">
				<td colspan="7" align="center"><%=pp%>&nbsp;<%=p%> <font
					color="black" style="font-size: 50px"> <%=String.format("%d년&nbsp;&nbsp;%d월", year, month)%>
				</font> <%=n%>&nbsp;<%=nn%></td>
			</tr>
			...
			<tr height="100" align="left" valign="top">
				...
				%>
				<td><%=reservelist(year, month, i)%>&nbsp;<%=showPen(year, month, i)%>
